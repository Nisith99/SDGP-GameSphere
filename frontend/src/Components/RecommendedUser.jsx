...
const RecommendedUser = ({ user }) => {
  ...
  const { mutate: cancelRequest } = useMutation({
    mutationFn: (userId) => axiosInstance.put(`/connections/cancel/${userId}`),
    onSuccess: () => {
  ...
  const renderButton = () => {
    ...
    switch (connectionStatus?.data?.status) {
      case "pending":
        return (
          <button
            className='px-3 py-1 rounded-full text-sm bg-yellow-500 text-white flex items-center hover:bg-yellow-600 transition-colors duration-200'
